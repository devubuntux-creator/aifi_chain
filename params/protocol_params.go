...
var (
	...
	MinBlocksForBlobRequests           uint64 = uint64(float64(MinTimeDurationForBlobRequests) / 0.50) // ref: https://github.com/bnb-chain/BEPs/blob/master/BEPs/BEP-524.md#421-change-table.
	DefaultExtraReserveForBlobRequests uint64 = uint64(24 * 3600 / 0.50)                               // it adds more time for expired blobs for some request cases, like expiry blob when remote peer is syncing, default 1 day.
